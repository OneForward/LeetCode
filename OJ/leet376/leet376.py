...
class Solution:
    def wiggleMaxLength(self, A: List[int]) -> int:
        ...
        for i in range(n):
            for j in range(i):
                if A[j] < A[i]: UP[i] = max(UP[i], DN[j]+1)
                if A[j] > A[i]: DN[i] = max(DN[i], UP[j]+1)
        return max(UP + DN)
...
class Solution:
    def wiggleMaxLength(self, A: List[int]) -> int:
        ...
        UP, DN = 1, 1
        for i in range(1,n):
            if A[i-1] < A[i]: UP = max(UP, DN+1); 
            if A[i-1] > A[i]: DN = max(DN, UP+1)
        return max(UP, DN)
