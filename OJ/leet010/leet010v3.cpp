...
tuple<bool, vector<State>> prepocess(const string& p) {
    ...
    while (i < p.length()) {
        if (i < p.length()-1  && p[i+1] == '*') {
            if (p[i] == '.') {
                v.push_back({Universal}); i += 2; continue;
            }
            if ( p[i] == '*' ) return {false, v};

            v.push_back({REPEATE, p[i]});  i += 2; continue;
        }
        ...
        {   
            ...
            if ( p[ip].state == REPEATE ) {
                ...
                for (int i=is; i<s.length(); ++i) {
                    if (s[i] == p[ip].data) {
                        if (isMatch(s, p, i+1, ip+1)) return true;
                    }
                    else break;
                }
                return isMatch(s, p, is, ip+1);
            }
