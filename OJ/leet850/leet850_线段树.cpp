...
long long update(int i, int l, int r, int L, int R, int val) {
    // (i, l, r) 第 i 个结点，指示区间 [l, r]
    // [L, R] query 区间
    if (L >= R) return tree[i].len;
    if (L == l and R == r) tree[i].cnt += val;
    else {
        int mid = (l + r) / 2;
        update(2*i, l,  mid, L, min(R, mid), val);
        update(2*i+1, mid, r, max(L, mid), R, val);
    }
    if (tree[i].cnt) tree[i].len = X[r] - X[l];
    else tree[i].len = tree[2*i].len + tree[2*i+1].len;
    return tree[i].len;
}
...
public:
    int rectangleArea(const vector<vector<int>>& rectangles) {
        // 扫描线 + 离散化 + 线段树, O(N log N), 4ms
        // 对 X 轴排序后离散化, 获得 [x1, x2, ..., xn]
        // 基于此构造结点值是区间的二叉树 [1, n] -> [1, n/2], [n/2, n], 区间长度至少为 1
        ...
        int n = 0;
        for (const auto& x: Xs) X[n] = x, M[x] = n, n++;

        int y1 = 0; long long ans = 0, len = 0;
        for (const auto& [y2, type, x1, x2]: events) {// N loops
            ans += len * (y2 - y1);  ans %= MOD;
            len = update(1, 0, n-1, M[x1], M[x2], type); // log N
            y1 = y2;
        }
        return ans;
    }
